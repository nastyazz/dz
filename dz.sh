...
# Если ключ для расшифровки не был передан как аргумент, шифруем файл и сохраняем ключ
if [[ $# -eq 1 ]]; then
    md5sum "$input_file" | cut -d' ' -f1 > "$key_file"
    openssl enc -aes-256-cbc -salt -in "$input_file" -out "$encrypted_file"
    echo "Файл '$input_file' успешно зашифрован и сохранен в '$encrypted_file'"
    echo "Ключ для расшифровки сохранен в '$key_file'"
    exit 0
fi

# Иначе пытаемся расшифровать файл с переданным ключом
if [[ $# -eq 2 ]]; then
    key=$2
else
    key=$(cat "$key_file")
...
echo "Файл '$encrypted_file' успешно расшифрован и сохранен в '$input_file'"
